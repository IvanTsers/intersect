...
options, prepare a subject and a query for the search, conduct the
...
#+end_src
#+begin_export latex
The program takes as input two \ty{fasta}-formatted sequences: a
subject and a query. The user can specify the minimum anchor length or
p-value of observing a non-random shustring.  We also declare options
for the output formatting, toggling the verbose mode and for printing
the version.
#+end_export
...
#+end_src
#+begin_export latex
We check whether the input file names are specified.
#+end_export
#+begin_src go <<Parse options>>=
  if !(*optQ != "" && *optS != "") {
...
\ty{MinAncLen()}. We print the minimum anchor length in the verbose
mode.
#+end_export
