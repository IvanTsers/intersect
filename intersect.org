...
of segregation sites \ty{numNs} if the segregation sites are present
in the headers.
#+end_export
...
#+end_src
#+begin_export latex
We search for the total number of segregation sites in the currect
sequence using a regular expression, then we convert the match to a
slice of bytes, get rid of the whitespace and semicolon, convert what
remains to an integer and add it to \ty{numNs}.
#+end_export
